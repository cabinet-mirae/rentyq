    ...
    <div class="rq-detail-v2">
      ...
      <section class="rq-detail-hero">
        ...
        <div class="rq-property-tabs">
          ...
          <button class="rq-property-tab" onclick="switchPropertyTab('market')"><span class="rq-property-tab-icon">\uD83C\uDF0D</span><span class="rq-property-tab-label">Market Intelligence</span></button>
        </div>
      </section>
      ...
      <div class="rq-property-tab-panel" id="rq-tab-market">
        <div id="rq-market-intel-content" class="rq-mi-loading">⏳ Analyse du quartier en cours…</div>
      </div>
...
function renderParcFiches(){
  ...
  var cards=apparts.map(function(a){
    ...
    var evaActionHtml='<div class="eva-section-label">Action EVA du jour</div>'+
      ...
      (evaActionBtn
       ? '<button class="eva-action-day-btn" onclick="' +
          (evaActionBtn === 'Voir CleanyQ'
            ? "goTo('cleanyq-operations', document.querySelector('[data-page=cleanyq-operations]'))"
            : "rqPreviousParcView='parc-fiches';goTo('parc',document.querySelector('[data-page=parc]'));setTimeout(function(){showApartDetail('" + a.id + "')},150)")
          + '">' + evaActionBtn + '</button>'
       : '')+
       '</div>'+
      '</div>';
...
async function syncEasyConcierge(){
  ...
  try{
    ...
    // puisque syncEasyConcierge a aussi besoin de les avoir à jour pour Parc EVA/Cockpit).
    if(isDemoMode())stopDemoMode();

    // Recharge apparts + reservations depuis Supabase (même requêtes que loadApp())
    const aRes=await sbFetch(`appartements?user_id=eq.${currentUser.user.id}&select=*&order=created_at.asc`);
    const allAppartsRaw=await aRes.json();
    const allApparts=Array.isArray(allAppartsRaw)?allAppartsRaw:[];
    apparts=allApparts.filter(a=>!a.archived);archivedApparts=allApparts.filter(a=>a.archived);
...
// ============================================================
// EVA MARKET INTELLIGENCE — Couche 2 Google Places (mockée)
//
// Totalement indépendant d'Easy Concierge, de pms_connections, et du moteur EVA existant
// (rqEvaPropertyHealth / calculatePropertyEva / rqComputeFinancials / les 3 sous-scores ne
// sont ni lus ni modifiés ici). N'écrit dans aucune table Supabase pour l'instant — tout
// vit en mémoire (cache + calcul à la demande), exactement comme demandé.
//
// Architecture pensée pour le futur vrai branchement Google Places : SEULE la fonction
// rqFetchPlacesData() devra être remplacée par un vrai appel API (Nearby Search autour de
// apartment.latitude/longitude) — calculateLocationAttractiveness(), le rendu et le cache
// n'auront rien à changer, à condition que le remplacement renvoie la même forme
// {restaurants_count, cafes_bars_count, attractions_count, commerces_count,
// transit_distance_m, parkings_count}.
// ============================================================

let rqMarketIntelCache={}; // { [apartmentId]: market_intelligence }

// ── PRNG déterministe (mulberry32), seedé par logement — jamais Math.random(). Un même
// logement doit toujours afficher la même analyse tant que ses données n'ont pas changé ;
// avec Math.random() le score changerait à chaque ouverture de fiche, ce qui ruinerait la
// crédibilité d'un "score de quartier" censé être stable. ──
function rqHashStringToSeed(str){
  let h=0;
  for(let i=0;i<str.length;i++){h=(h*31+str.charCodeAt(i))|0;}
  return h>>>0;
}
function rqMulberry32(seed){
  let a=seed>>>0;
  return function(){
    a=(a+0x6D2B79F5)|0;
...
// ── Couche d'accès aux lieux — MOCKÉE pour l'instant. Seule cette fonction devra être
// remplacée par un vrai appel Google Places Nearby Search plus tard (voir en-tête de
// section). Les valeurs sont déterministes (seedées par logement) et orientées par le
// contexte connu (zone "centre" boostée) pour rester plausibles, jamais du bruit pur. ──
async function rqFetchPlacesData(apartment){
  const seedStr=String(apartment.id||'')+'|'+String(apartment.address||'')+'|'+String(apartment.city||'');
  const rand=rqMulberry32(rqHashStringToSeed(seedStr));
  const isCentre=/centre|centre-ville|hyper-centre/i.test(String(apartment.zone||''))||/centre/i.test(String(apartment.address||''));
  const centreBoost=isCentre?1:0;

  return {
    restaurants_count:Math.round(6+rand()*16+centreBoost*14),
    cafes_bars_count:Math.round(2+rand()*8+centreBoost*6),
    attractions_count:Math.round(rand()*5+centreBoost*4),
    commerces_count:Math.round(4+rand()*14+centreBoost*8),
    transit_distance_m:Math.max(80,Math.round(180+rand()*1300-centreBoost*550)),
    parkings_count:Math.round(rand()*4+1+centreBoost)
  };
}

// === calculateLocationAttractiveness(apartment) ===
// Retourne un objet market_intelligence (voir structure ci-dessous), ou un état explicite
// "coordonnées manquantes" si latitude/longitude ne sont pas renseignées — jamais de score
// inventé sur des coordonnées qui n'existent pas.
async function calculateLocationAttractiveness(apartment){
  if(!apartment)return null;

  const hasCoords=apartment.latitude!=null&&apartment.latitude!==''&&apartment.longitude!=null&&apartment.longitude!=='';
  if(!hasCoords){
    return{
      hasCoords:false,
      message:'Coordonnées manquantes',
      canUseAddressLater:!!(apartment.address||apartment.city)
    };
  }

  const cacheKey=String(apartment.id);
  if(rqMarketIntelCache[cacheKey])return rqMarketIntelCache[cacheKey];

  const places=await rqFetchPlacesData(apartment);

  // Sous-scores 0-100, plafonnés. Pondération du score de quartier sur les 6 dimensions
  // explicitement demandées (restaurants, cafés/bars, attractions, transport, parkings,
  // commerces) — la distance centre-ville n'est volontairement PAS intégrée : on n'a
  // aucune référence fiable de "centre-ville" par coordonnées à ce stade (pas de base de
  // données de centres-villes), donc on ne fabrique pas un chiffre dessus.
  const restaurantsScore=Math.min(100,Math.round(places.restaurants_count*3.0));
  const cafesScore=Math.min(100,Math.round(places.cafes_bars_count*7));
  const attractionsScore=Math.min(100,Math.round(places.attractions_count*15));
  const transportScore=Math.max(0,Math.min(100,Math.round(100-places.transit_distance_m/15)));
  const parkingScore=Math.min(100,Math.round(places.parkings_count*16));
  const commerceScore=Math.min(100,Math.round(places.commerces_count*5));

  const location_score=Math.round(
    restaurantsScore*0.22+
    cafesScore*0.13+
    attractionsScore*0.20+
    transportScore*0.20+
    parkingScore*0.12+
    commerceScore*0.13
  );

  // event_score : signal RÉEL (pas mocké) — réutilise eventsCache déjà alimenté par
  // loadEvents() (Ticketmaster/OpenAgenda) pour la ville du logement. Volontairement gardé
  // hors du calcul de location_score (pas demandé dans la liste des 6 critères), affiché à
  // part comme un signal de pression événementielle locale.
  const cityEvents=(typeof eventsCache!=='undefined'?eventsCache[apartment.city]:null)||[];
  const hotEventsCount=cityEvents.filter(e=>e&&e.hot).length;
  const event_score=Math.min(100,hotEventsCount*25);

  // Impact tarifaire : un quartier mieux noté justifie un ADR de marché plus élevé que le
  // prix actuel. Mock structuré pour rester remplaçable par une vraie donnée de marché
  // (comparables OTA) plus tard, sans changer la forme du résultat.
  const basePrice=Number(apartment.price)||Number(apartment.ai_rec)||60;
  const marketUplift=(location_score-50)/100; // -0.5 .. +0.5
  const estimated_market_adr=Math.round(basePrice*(1+marketUplift*0.35));
  const price_gap=estimated_market_adr-Math.round(basePrice);
  const nightly_potential=Math.max(0,price_gap);

  const strengths=[],weaknesses=[];
  if(places.restaurants_count>=15)strengths.push(`${places.restaurants_count} restaurants à moins de 500 m`);
  else if(places.restaurants_count<=6)weaknesses.push(`Seulement ${places.restaurants_count} restaurant(s) à proximité`);
  if(places.attractions_count>=4)strengths.push(`${places.attractions_count} attractions touristiques à proximité`);
  else if(places.attractions_count===0)weaknesses.push('Aucune attraction touristique recensée à proximité');
  if(places.transit_distance_m<=1000)strengths.push(`Gare/transport en commun à ${places.transit_distance_m} m`);
  else weaknesses.push(`Transport en commun éloigné (${places.transit_distance_m} m)`);
  if(places.parkings_count>=3)strengths.push(`${places.parkings_count} parkings à proximité`);
  else if(places.parkings_count===0)weaknesses.push('Aucun parking recensé à proximité');
  if(hotEventsCount>0)strengths.push(`${hotEventsCount} événement(s) à forte affluence détecté(s) à proximité`);
  if(location_score>=75)strengths.push('Quartier très favorable aux courts séjours');
  else if(location_score<45)weaknesses.push('Positionnement excentré, moins recherché en courts séjours');

  let recommendation;
  if(location_score>=80)recommendation=`Quartier premium — un tarif au-dessus du marché est justifiable${nightly_potential>0?` (+${nightly_potential}€/nuit)`:''}.`;
  else if(location_score>=60)recommendation='Quartier porteur — bien positionné pour la demande courts séjours.';
  else if(location_score>=40)recommendation='Quartier correct mais sans atout fort — misez sur le rapport qualité-prix plutôt que sur l\u2019emplacement.';
  else recommendation='Quartier excentré — compensez par le prix ou la qualité du logement plutôt que par l\u2019emplacement.';

  const result={
    hasCoords:true,
    location_score,
    restaurants_count:places.restaurants_count,
    attractions_count:places.attractions_count,
    transport_score:transportScore,
    parking_score:parkingScore,
    event_score,
    estimated_market_adr,
    price_gap,
    nightly_potential,
    strengths,
    weaknesses,
  ...
  rqMarketIntelCache[cacheKey]=result;
  return result;
}
...
// Rendu de l'onglet "Market Intelligence" — chargement à la demande (au premier clic sur
// l'onglet), résultat caché ensuite tant que le logement n'a pas changé.
async function rqRenderMarketIntelligencePanel(apartmentId){
  const el=document.getElementById('rq-market-intel-content');
  if(!el)return;
  const a=apparts.find(x=>String(x.id)===String(apartmentId));
  if(!a){el.innerHTML='Logement introuvable.';return;}

  el.className='rq-mi-loading';
  el.innerHTML='⏳ Analyse du quartier en cours…';

  let mi;
  try{
    mi=await calculateLocationAttractiveness(a);
  }catch(e){
    el.className='rq-mi-empty';
    el.innerHTML='Impossible de calculer l\u2019analyse de quartier pour ce logement.';
    return;
  }
  ...
  if(!mi||!mi.hasCoords){
    el.className='rq-mi-empty';
    el.innerHTML=`
      <div class="rq-mi-empty-icon">📍</div>
      <div class="rq-mi-empty-title">Coordonnées manquantes</div>
      <div class="rq-mi-empty-sub">EVA a besoin de la latitude/longitude du logement pour analyser son quartier.</div>
      ${mi&&mi.canUseAddressLater?`<button class="btn btn-purple" onclick="openEdit('${a.id}')">📍 Renseigner les coordonnées</button>`:''}
    `;
    return;
  }

  const scoreClass=mi.location_score>=80?'pepites':mi.location_score>=60?'surveiller':mi.location_score>=40?'sous_perf':'arbitrer';

  el.className='';
  el.innerHTML=`
    <div class="rq-mi-header">
      <div>
        <div class="rq-mi-title">Attractivité quartier</div>
        <div class="rq-mi-sub">${rqMiEsc(a.address||a.city||'')}</div>
      </div>
      <div class="rq-mi-score-badge ${scoreClass}">${mi.location_score}<span>/100</span></div>
    </div>

    <div class="rq-mi-columns">
      <div class="rq-mi-col">
        <div class="rq-mi-col-title">✅ Points forts</div>
        ${mi.strengths.length?mi.strengths.map(s=>`<div class="rq-mi-point good">${rqMiEsc(s)}</div>`).join(''):'<div class="rq-mi-point-empty">Aucun point fort identifié.</div>'}
      </div>
      <div class="rq-mi-col">
        <div class="rq-mi-col-title">⚠️ Points faibles</div>
        ${mi.weaknesses.length?mi.weaknesses.map(s=>`<div class="rq-mi-point bad">${rqMiEsc(s)}</div>`).join(''):'<div class="rq-mi-point-empty">Aucun point faible identifié.</div>'}
      </div>
    </div>

    <div class="rq-mi-impact">
      <div>
        <div class="rq-mi-impact-label">Impact EVA</div>
        <div class="rq-mi-impact-value">${mi.nightly_potential>0?'+'+mi.nightly_potential+'€/nuit potentiel':'Prix déjà aligné avec le marché local'}</div>
      </div>
      <div class="rq-mi-impact-detail">ADR de marché estimé : <b>${mi.estimated_market_adr}€</b> · Prix actuel : <b>${Math.round(Number(a.price)||0)}€</b></div>
    </div>

    <div class="rq-mi-reco">💡 ${rqMiEsc(mi.recommendation)}</div>

    <div class="rq-mi-footnote">⚠️ Données de quartier simulées (mock intelligent) — l\u2019intégration Google Places réelle remplacera cette estimation sans changer cet écran.</div>
  `;
}
...
// ============================================================
// MODE DÉMO EVA — Sprint Démo : dataset statique copié depuis le compte démo interne
// contact@rentyq.fr (12 logements, 256 réservations, 179 charges, 67 missions — extraits
// le 2026-06-26). Jamais de lecture directe de ce compte à l'exécution, jamais d'écriture
// en base : ce mode ne remplace QUE les variables front (apparts, reservations,
// chargesData, missionsData, reportsData, reviewsData si ce dernier existe un jour).
//
// reportsData et reviewsData sont vides ci-dessous : le compte démo réel n'a aujourd'hui
// aucun cleaning_reports ni reviews — fidèle à la réalité plutôt qu'inventé.
//
// Les dates des réservations/missions sont stockées en décalage de jours par rapport à une
// ancre fixe (RQ_DEMO_ANCHOR_DATE = la date du jour au moment de l'extraction). Au chargement,
// on recalcule le décalage entre "aujourd'hui" réel et cette ancre et on l'applique à toutes
// les dates : le dataset reste un copié-figé de valeurs réelles (rien n'est régénéré ni
// randomisé), mais ne devient jamais visuellement obsolète, même des mois après ce sprint.

const RQ_DEMO_ANCHOR_DATE = '2026-06-26';
const RQ_DEMO_APARTMENTS = [{"id": "demo-apt-1", "name": "Studio Cathédrale", "city": "Orléans", "zone": "Centre-ville", "address": "5 rue Jeanne d'Arc, Orléans", "emoji": "⛪", "rent": 550.0, "cleaner": 35.0, "price": 58.0, "comp": 55.0, "ai_rec": 63.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.6, "nb_avis": 0, "bedrooms": 0, "bathrooms": null, "max_guests": 2, "surface_m2": 28.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.9025, "longitude": 1.9087}, {"id": "demo-apt-2", "name": "Le Clos des Bords de Loire", "city": "Saint-Jean-le-Blanc", "zone": "Bords de Loire", "address": "8 allée des Peupliers, Saint-Jean-le-Blanc", "emoji": "🌊", "rent": 1100.0, "cleaner": 90.0, "price": 145.0, "comp": 138.0, "ai_rec": 167.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.5, "nb_avis": 0, "bedrooms": 4, "bathrooms": null, "max_guests": 8, "surface_m2": 120.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.8835, "longitude": 1.918}, {"id": "demo-apt-3", "name": "Le Saint-Marceau", "city": "Orléans", "zone": "Saint-Marceau", "address": "44 avenue Saint-Marceau, Orléans", "emoji": "🏘️", "rent": 870.0, "cleaner": 60.0, "price": 98.0, "comp": 93.0, "ai_rec": 116.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.4, "nb_avis": 0, "bedrooms": 2, "bathrooms": null, "max_guests": 6, "surface_m2": 68.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.8965, "longitude": 1.9145}, {"id": "demo-apt-4", "name": "Le Cosy Dunois", "city": "Orléans", "zone": "Quartier Dunois", "address": "17 rue Dunois, Orléans", "emoji": "🏡", "rent": 680.0, "cleaner": 45.0, "price": 72.0, "comp": 68.0, "ai_rec": 79.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.2, "nb_avis": 0, "bedrooms": 1, "bathrooms": null, "max_guests": 4, "surface_m2": 42.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.907, "longitude": 1.8965}, {"id": "demo-apt-5", "name": "L'Escapade Jeanne", "city": "Orléans", "zone": "Centre-ville", "address": "12 rue Royale, Orléans", "emoji": "🏛️", "rent": 950.0, "cleaner": 65.0, "price": 97.0, "comp": 92.0, "ai_rec": 99.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.9, "nb_avis": 0, "bedrooms": 2, "bathrooms": null, "max_guests": 4, "surface_m2": 52.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.9015, "longitude": 1.905}, {"id": "demo-apt-6", "name": "Duplex République", "city": "Orléans", "zone": "Centre-ville", "address": "3 place de la République, Orléans", "emoji": "🏢", "rent": 980.0, "cleaner": 75.0, "price": 108.0, "comp": 105.0, "ai_rec": 118.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.7, "nb_avis": 0, "bedrooms": 3, "bathrooms": null, "max_guests": 6, "surface_m2": 85.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.904, "longitude": 1.9095}, {"id": "demo-apt-7", "name": "L'Atelier Gambetta", "city": "Orléans", "zone": "Gambetta", "address": "22 rue Gambetta, Orléans", "emoji": "🎨", "rent": 750.0, "cleaner": 50.0, "price": 85.0, "comp": 80.0, "ai_rec": 91.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.6, "nb_avis": 0, "bedrooms": 1, "bathrooms": null, "max_guests": 4, "surface_m2": 48.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.908, "longitude": 1.912}, {"id": "demo-apt-8", "name": "Le Reflet de Loire", "city": "Orléans", "zone": "Bords de Loire", "address": "1 quai Barentin, Orléans", "emoji": "✨", "rent": 1200.0, "cleaner": 95.0, "price": 155.0, "comp": 148.0, "ai_rec": 160.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.9, "nb_avis": 0, "bedrooms": 3, "bathrooms": null, "max_guests": 6, "surface_m2": 105.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.905, "longitude": 1.9135}, {"id": "demo-apt-9", "name": "Le Quai Royal", "city": "Orléans", "zone": "Bords de Loire", "address": "2 quai du Roi, Orléans", "emoji": "⚓", "rent": 920.0, "cleaner": 70.0, "price": 109.0, "comp": 112.0, "ai_rec": 116.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.7, "nb_avis": 0, "bedrooms": 2, "bathrooms": null, "max_guests": 5, "surface_m2": 72.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.9038, "longitude": 1.907}, {"id": "demo-apt-10", "name": "Le Petit Sologne", "city": "La Source", "zone": "La Source", "address": "6 rue des Iris, La Source", "emoji": "🌲", "rent": 780.0, "cleaner": 55.0, "price": 88.0, "comp": 82.0, "ai_rec": 104.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.5, "nb_avis": 0, "bedrooms": 2, "bathrooms": null, "max_guests": 6, "surface_m2": 65.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.847, "longitude": 1.933}, {"id": "demo-apt-11", "name": "Le Faubourg Bannier", "city": "Orléans", "zone": "Faubourg Bannier", "address": "31 rue du Faubourg Bannier, Orléans", "emoji": "🏠", "rent": 640.0, "cleaner": 40.0, "price": 68.0, "comp": 64.0, "ai_rec": 74.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.1, "nb_avis": 0, "bedrooms": 1, "bathrooms": null, "max_guests": 4, "surface_m2": 38.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.9145, "longitude": 1.901}, {"id": "demo-apt-12", "name": "Le Jardin de Jeanne", "city": "Orléans", "zone": "Faubourg Bourgogne", "address": "9 rue du Jardin des Plantes, Orléans", "emoji": "🌿", "rent": 1050.0, "cleaner": 80.0, "price": 124.0, "comp": 128.0, "ai_rec": 130.0, "booked": true, "auto_pricing": true, "has_event": false, "auto_degressif": false, "degressif_start": 14, "degressif_step": 5.0, "degressif_min": 0.0, "note": 4.7, "nb_avis": 0, "bedrooms": 3, "bathrooms": null, "max_guests": 6, "surface_m2": 95.0, "user_id": "demo", "proprietaire_id": null, "source": null, "external_id": null, "main_photo_url": null, "amenities_json": [], "latitude": 47.9095, "longitude": 1.9165}];
const RQ_DEMO_RESERVATIONS_RAW = [{"appartement_id": "demo-apt-7", "guest_name": "Kevin T.", "date_from_offset": -42, "date_to_offset": -40, "nights": 2, "price_total": 220.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-3", "guest_name": "Pierre V.", "date_from_offset": -42, "date_to_offset": -39, "nights": 3, "price_total": 381.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-12", "guest_name": "François D.", "date_from_offset": -42, "date_to_offset": -39, "nights": 3, "price_total": 535.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-5", "guest_name": "Amélie F.", "date_from_offset": -41, "date_to_offset": -39, "nights": 2, "price_total": 246.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-4", "guest_name": "Pierre V.", "date_from_offset": -41, "date_to_offset": -38, "nights": 3, "price_total": 279.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-6", "guest_name": "Kevin T.", "date_from_offset": -41, "date_to_offset": -38, "nights": 3, "price_total": 484.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-9", "guest_name": "Benjamin G.", "date_from_offset": -41, "date_to_offset": -38, "nights": 3, "price_total": 481.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-1", "guest_name": "Julien M.", "date_from_offset": -41, "date_to_offset": -38, "nights": 3, "price_total": 225.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-11", "guest_name": "Nicolas G.", "date_from_offset": -40, "date_to_offset": -36, "nights": 4, "price_total": 300.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-8", "guest_name": "Pierre V.", "date_from_offset": -40, "date_to_offset": -36, "nights": 4, "price_total": 684.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-7", "guest_name": "Léa R.", "date_from_offset": -40, "date_to_offset": -37, "nights": 3, "price_total": 282.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-12", "guest_name": "Romain D.", "date_from_offset": -39, "date_to_offset": -34, "nights": 5, "price_total": 745.0, "platform": "direct", "status": "confirmed"}, {"appartement_id": "demo-apt-3", "guest_name": "Thomas B.", "date_from_offset": -39, "date_to_offset": -36, "nights": 3, "price_total": 324.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-2", "guest_name": "Sophie T.", "date_from_offset": -39, "date_to_offset": -35, "nights": 4, "price_total": 640.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-5", "guest_name": "
0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-7", "guest_name": "Lucas P.", "date_from_offset": 36, "date_to_offset": 38, "nights": 2, "price_total": 236.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-3", "guest_name": "François D.", "date_from_offset": 36, "date_to_offset": 39, "nights": 3, "price_total": 349.0, "platform": "direct", "status": "confirmed"}, {"appartement_id": "demo-apt-4", "guest_name": "Claire M.", "date_from_offset": 36, "date_to_offset": 39, "nights": 3, "price_total": 249.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-12", "guest_name": "Antoine R.", "date_from_offset": 36, "date_to_offset": 38, "nights": 2, "price_total": 312.0, "platform": "direct", "status": "confirmed"}, {"appartement_id": "demo-apt-5", "guest_name": "Céline V.", "date_from_offset": 36, "date_to_offset": 39, "nights": 3, "price_total": 385.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-11", "guest_name": "Jean-Pierre D.", "date_from_offset": 36, "date_to_offset": 39, "nights": 3, "price_total": 247.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-10", "guest_name": "Camille H.", "date_from_offset": 36, "date_to_offset": 38, "nights": 2, "price_total": 204.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-1", "guest_name": "Maxime B.", "date_from_offset": 36, "date_to_offset": 38, "nights": 2, "price_total": 134.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-12", "guest_name": "Romain D.", "date_from_offset": 38, "date_to_offset": 45, "nights": 7, "price_total": 934.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-2", "guest_name": "Kevin T.", "date_from_offset": 38, "date_to_offset": 45, "nights": 7, "price_total": 994.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-1", "guest_name": "Sophie T.", "date_from_offset": 38, "date_to_offset": 45, "nights": 7, "price_total": 409.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-10", "guest_name": "Anaïs B.", "date_from_offset": 38, "date_to_offset": 46, "nights": 8, "price_total": 688.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-7", "guest_name": "Pierre V.", "date_from_offset": 38, "date_to_offset": 45, "nights": 7, "price_total": 591.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-8", "guest_name": "Chloé M.", "date_from_offset": 38, "date_to_offset": 43, "nights": 5, "price_total": 760.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-5", "guest_name": "Lucas P.", "date_from_offset": 39, "date_to_offset": 46, "nights": 7, "price_total": 756.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-6", "guest_name": "Marie L.", "date_from_offset": 39, "date_to_offset": 46, "nights": 7, "price_total": 871.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-3", "guest_name": "Hugo S.", "date_from_offset": 39, "date_to_offset": 44, "nights": 5, "price_total": 480.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-4", "guest_name": "Anaïs B.", "date_from_offset": 39, "date_to_offset": 46, "nights": 7, "price_total": 497.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-9", "guest_name": "Chloé M.", "date_from_offset": 40, "date_to_offset": 45, "nights": 5, "price_total": 580.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-11", "guest_name": "Camille H.", "date_from_offset": 40, "date_to_offset": 48, "nights": 8, "price_total": 558.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-8", "guest_name": "Léa R.", "date_from_offset": 43, "date_to_offset": 45, "nights": 2, "price_total": 358.0, "platform": "booking", "status": "confirmed"}, {"appartement_id": "demo-apt-3", "guest_name": "Camille H.", "date_from_offset": 44, "date_to_offset": 51, "nights": 7, "price_total": 704.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-8", "guest_name": "Isabelle F.", "date_from_offset": 45, "date_to_offset": 51, "nights": 6, "price_total": 944.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-2", "guest_name": "Céline V.", "date_from_offset": 45, "date_to_offset": 52, "nights": 7, "price_total": 1004.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-9", "guest_name": "Anaïs B.", "date_from_offset": 46, "date_to_offset": 53, "nights": 7, "price_total": 822.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-10", "guest_name": "Pierre V.", "date_from_offset": 46, "date_to_offset": 53, "nights": 7, "price_total": 602.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-12", "guest_name": "Isabelle F.", "date_from_offset": 46, "date_to_offset": 50, "nights": 4, "price_total": 528.0, "platform": "direct", "status": "confirmed"}, {"appartement_id": "demo-apt-4", "guest_name": "Anaïs B.", "date_from_offset": 46, "date_to_offset": 50, "nights": 4, "price_total": 284.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-5", "guest_name": "Kevin T.", "date_from_offset": 46, "date_to_offset": 53, "nights": 7, "price_total": 661.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-6", "guest_name": "Isabelle F.", "date_from_offset": 46, "date_to_offset": 53, "nights": 7, "price_total": 861.0, "platform": "direct", "status": "confirmed"}, {"appartement_id": "demo-apt-1", "guest_name": "Sarah L.", "date_from_offset": 46, "date_to_offset": 51, "nights": 5, "price_total": 285.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-7", "guest_name": "Lucie K.", "date_from_offset": 46, "date_to_offset": 52, "nights": 6, "price_total": 498.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-11", "guest_name": "Antoine R.", "date_from_offset": 48, "date_to_offset": 56, "nights": 8, "price_total": 632.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-4", "guest_name": "Julien M.", "date_from_offset": 50, "date_to_offset": 52, "nights": 2, "price_total": 198.0, "platform": "airbnb", "status": "confirmed"}, {"appartement_id": "demo-apt-12", "guest_name": "Benjamin G.", "date_from_offset": 50, "date_to_offset": 53, "nights": 3, "price_total": 468.0, "platform": "direct", "status": "confirmed"}];
const RQ_DEMO_CHARGES = [{"appartement_id": "demo-apt-5", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 65.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 19.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Exceptionnel", "label": "Remplacement jeu de clés magnétiques", "amount": 85.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-5", "category": "Exceptionnel", "label": "Ménage renforcé après séjour famille", "amount": 45.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-5", "category": "Exceptionnel", "label": "Réparation robinet salle de bain", "amount": 120.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-5", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 9.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 950.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 18.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 14.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-5", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 11.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 1100.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 66.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 22.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 26.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 22.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 42.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Exceptionnel", "label": "Réparation portail jardin", "amount": 220.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-2", "category": "Exceptionnel", "label": "Serrurerie urgence week-end", "amount": 185.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-2", "category": "Exceptionnel", "label": "Remplacement vaisselle cassée", "amount": 65.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-2", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 90.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 36.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-2", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 18.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 19.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 47.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 75.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 980.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 30.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Exceptionnel", "label": "Ménage renforcé post-groupe", "amount": 55.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-6", "category": "Exceptionnel", "label": "Remplacement lave-linge", "amount": 480.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-6", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 27.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-6", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 10.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Exceptionnel", "label": "Débouchage canalisation", "amount": 95.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-1", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 550.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 35.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 5.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 6.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 11.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Exceptionnel", "label": "Remplacement clés perdues", "amount": 55.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-1", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-1", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 13.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 17.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 13.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 9.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 23.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 45.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 680.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-4", "category": "Exceptionnel", "label": "Remplacement store brisé", "amount": 145.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-4", "category": "Exceptionnel", "label": "Bricolage divers (fixation, peinture)", "amount": 75.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-4", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 11.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 26.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 18.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Exceptionnel", "label": "Serrurerie remplacement cylindre", "amount": 155.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-7", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 50.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 750.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 17.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 14.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-7", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 9.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 870.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 60.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 37.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 16.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 24.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Exceptionnel", "label": "Remplacement matelas chambre 2", "amount": 340.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-3", "category": "Exceptionnel", "label": "Réparation chauffe-eau", "amount": 260.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-3", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 23.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-3", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 1050.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 80.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 52.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 17.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 21.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Exceptionnel", "label": "Entretien jardin renforcé", "amount": 180.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-12", "category": "Exceptionnel", "label": "Remplacement barbecue", "amount": 120.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-12", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 19.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 33.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-12", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 30.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 16.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Exceptionnel", "label": "Réparation volet roulant électrique", "amount": 195.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-11", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 13.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 13.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 7.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 21.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 40.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 640.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-11", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 25.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Exceptionnel", "label": "Remplacement TV salon", "amount": 399.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-9", "category": "Exceptionnel", "label": "Ménage renforcé post festival Loire", "amount": 65.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-9", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 17.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 16.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 13.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 40.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 70.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 920.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 24.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-9", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 36.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 780.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 55.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 14.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 22.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 16.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 23.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-10", "category": "Exceptionnel", "label": "Bricolage terrasse bois", "amount": 145.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-8", "category": "Exceptionnel", "label": "Réfection peinture salon", "amount": 680.0, "type": "variable", "per": "une_fois"}, {"appartement_id": "demo-apt-8", "category": "Loyer", "label": "Loyer mensuel propriétaire", "amount": 1200.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "PMS", "label": "Abonnement PMS et outils de gestion", "amount": 12.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Comptabilité", "label": "Quote-part honoraires comptables", "amount": 8.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Frais bancaires", "label": "Quote-part frais de tenue de compte et virements", "amount": 4.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Linge", "label": "Location et blanchisserie linge de maison", "amount": 32.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Ménage", "label": "Forfait ménage entre locataires", "amount": 95.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Plateforme", "label": "Abonnement logiciel conciergerie", "amount": 15.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Eau", "label": "Abonnement eau et assainissement", "amount": 19.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Assurance", "label": "Assurance propriétaire non occupant", "amount": 23.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Maintenance", "label": "Provision maintenance préventive", "amount": 37.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Consommables", "label": "Savon, shampoing, café, papier", "amount": 21.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Internet", "label": "Abonnement fibre optique", "amount": 29.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Électricité", "label": "Contrat électricité EDF", "amount": 58.0, "type": "fixe", "per": "mois"}, {"appartement_id": "demo-apt-8", "category": "Exceptionnel", "label": "Remplacement canapé cuir", "amount": 890.0, "type": "variable", "per": "une_fois"}];
const RQ_DEMO_MISSIONS_RAW = [{"appartement_id": "demo-apt-3", "date_offset": -10, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-1", "date_offset": -9, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-7", "date_offset": -9, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-2", "date_offset": -9, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-9", "date_offset": -8, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-4", "date_offset": -8, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-12", "date_offset": -8, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-10", "date_offset": -8, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-3", "date_offset": -7, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-5", "date_offset": -7, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-11", "date_offset": -7, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-8", "date_offset": -6, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-2", "date_offset": -6, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-12", "date_offset": -5, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-6", "date_offset": -5, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-1", "date_offset": -5, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-7", "date_offset": -5, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-3", "date_offset": -5, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-9", "date_offset": -5, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-11", "date_offset": -4, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-2", "date_offset": -4, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_cours"}, {"appartement_id": "demo-apt-10", "date_offset": -3, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-4", "date_offset": -3, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-5", "date_offset": -3, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-1", "date_offset": -2, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-8", "date_offset": -2, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-3", "date_offset": -2, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-11", "date_offset": -1, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-6", "date_offset": 0, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-2", "date_offset": 0, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-7", "date_offset": 0, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-9", "date_offset": 0, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-12", "date_offset": 1, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-4", "date_offset": 1, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-10", "date_offset": 1, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-3", "date_offset": 1, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-2", "date_offset": 2, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-11", "date_offset": 2, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-5", "date_offset": 2, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-1", "date_offset": 2, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-7", "date_offset": 2, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-9", "date_offset": 2, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-7", "date_offset": 3, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-6", "date_offset": 3, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-8", "date_offset": 3, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-3", "date_offset": 3, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-11", "date_offset": 4, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-4", "date_offset": 4, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-10", "date_offset": 4, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-9", "date_offset": 5, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-5", "date_offset": 5, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-6", "date_offset": 5, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-1", "date_offset": 5, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-12", "date_offset": 5, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-5", "date_offset": 10, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-7", "date_offset": 10, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-4", "date_offset": 10, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-10", "date_offset": 10, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-11", "date_offset": 11, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-1", "date_offset": 11, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}, {"appartement_id": "demo-apt-2", "date_offset": 11, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-6", "date_offset": 12, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-9", "date_offset": 12, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-3", "date_offset": 12, "heure": "10:00", "duree_min": 150, "tarif": 65.0, "status": "en_attente"}, {"appartement_id": "demo-apt-12", "date_offset": 13, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-8", "date_offset": 13, "heure": "10:00", "duree_min": 180, "tarif": 90.0, "status": "en_attente"}, {"appartement_id": "demo-apt-5", "date_offset": 14, "heure": "10:00", "duree_min": 90, "tarif": 40.0, "status": "en_attente"}];

function rqDemoShiftDays(){
  const anchor=new Date(RQ_DEMO_ANCHOR_DATE+'T00:00:00');
  const today=new Date();today.setHours(0,0,0,0);
  return Math.round((today-anchor)/86400000);
}
function rqDemoDateFromOffset(offsetDays,shiftDays){
  const d=new Date(RQ_DEMO_ANCHOR_DATE+'T00:00:00');
  d.setDate(d.getDate()+offsetDays+shiftDays);
  return d.toISOString().slice(0,10);
}

// === isDemoMode() ===
function isDemoMode(){
  try{return localStorage.getItem('rentyq_demo_mode')==='true';}catch(e){return false;}
}

// === loadDemoDataset() ===
// Ne remplace QUE les variables front listées — jamais d'écriture Supabase, jamais de
// lecture du compte contact@rentyq.fr à l'exécution (tout est déjà figé ci-dessus).
function loadDemoDataset(){
  const shift=rqDemoShiftDays();

  apparts=RQ_DEMO_APARTMENTS.map(a=>({...a}));
  archivedApparts=[];

  reservations=RQ_DEMO_RESERVATIONS_RAW.map(r=>({
    appartement_id:r.appartement_id,
    guest_name:r.guest_name,
    date_from:rqDemoDateFromOffset(r.date_from_offset,shift),
    date_to:rqDemoDateFromOffset(r.date_to_offset,shift),
    nights:r.nights,
  ...
  if(typeof chargesData!=='undefined')chargesData=RQ_DEMO_CHARGES.map(c=>({...c}));

  if(typeof missionsData!=='undefined')missionsData=RQ_DEMO_MISSIONS_RAW.map(m=>({
    appartement_id:m.appartement_id,
    date:rqDemoDateFromOffset(m.date_offset,shift),
    heure:m.heure,
  ...
  // Fidèle au compte démo réel : 0 cleaning_reports, 0 reviews aujourd'hui.
  if(typeof reportsData!=='undefined')reportsData=[];
  if(typeof reviewsData!=='undefined')reviewsData=[];
}

// === startDemoMode() ===
function startDemoMode(){
  try{localStorage.setItem('rentyq_demo_mode','true');}catch(e){}
  loadDemoDataset();
...
// === stopDemoMode() ===
// Si un utilisateur réel est connecté, recharge ses vraies données Supabase (même requête
// que loadApp()). Si la démo a été lancée sans compte (depuis l'écran de connexion),
// ramène simplement à l'écran de connexion — il n'y a pas de vraies données à recharger.
function stopDemoMode(){
  try{localStorage.removeItem('rentyq_demo_mode');}catch(e){}
  renderDemoBanner();
...
function renderDemoBanner(){
  const existing=document.getElementById('rq-demo-banner');
  if(!isDemoMode()){
    if(existing)existing.remove();
    if(document.body)document.body.classList.remove('rq-demo-active');
    return;
  }
  if(document.body)document.body.classList.add('rq-demo-active');
  if(existing)return;
  const banner=document.createElement('div');
  banner.id='rq-demo-banner';
  banner.className='rq-demo-banner';
  banner.innerHTML='\u{1F680} Vous explorez RentyQ avec des donn\u00e9es de d\u00e9monstration \u2014 rien n\u2019est r\u00e9el, rien n\u2019est sauvegard\u00e9. <button type="button" onclick="stopDemoMode()">Quitter le mode d\u00e9mo</button>';
...
function switchPropertyTab(tab){
  ...
  const idx={overview:0,audit:1,pricing:2,calendar:3,charges:4,market:5}[tab]||0;
  if(btns[idx])btns[idx].classList.add('active');
  const panel=document.getElementById('rq-tab-'+tab);
  if(panel)panel.classList.add('active');
  if(tab==='market')rqRenderMarketIntelligencePanel(rqCurrentDetailApartId);
}
